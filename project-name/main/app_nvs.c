...
///	Purpose: NVS configuration file
...
bool app_nvs_load_sta_creds(void)
{
    ...
    if(nvs_open(app_nvs_sta_creds_namespace, NVS_READONLY, &handle) == ESP_OK)
    {
        ...
        esp_err = nvs_get_blob(handle, "password", wifi_config_buff, &wifi_config_size); //using "password" key to load the password into the wifi_config_buff
        if(esp_err != ESP_OK)
        {
            free(wifi_config_buff);
        ...
        memcpy(wifi_sta_config->sta.password, wifi_config_buff, wifi_config_size); //if successful, we copy the password into the sta.password config
        
        free(wifi_config_buff);
        nvs_close(handle);

        printf("app_nvs_load_sta_creds: SSID %s Password %s\n", wifi_sta_config->sta.ssid, wifi_sta_config->sta.password);
        return (wifi_sta_config->sta.ssid[0] != '\0'); //if the first letter is not a null terminator then it's a valid ssid
    } 
    else
    {
        return false;
