...
///	Purpose: HTTP configuration and startup file
...
#include "esp_ota_ops.h" //for OTA functions
...
//Embedded files: JQuery, index.html, app.css, app.js, favicon.ico
...
/*
    Checks the g_fw_update_status and creates the fw_update_rest timer if g_fw_update_Status is true
*/
...
/*
    ...
    req = HTTP request for which the uri needs to be handled
    return = ESP_OK
*/
static esp_err_t http_server_favicon_icon_handler(httpd_req_t *req )
{
...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
    ...
    char ota_buff[1024]; //buffer to hold data received from the web page
    int content_length = req->content_len; //
    int content_received = 0;

    int recv_len; //receives data from each HTTP request function call
    bool is_req_body_started = false; //variable to indicate when the actual firmware update file content is found
    ...
    do
    {
        ...
        //if so, it will have the information handle if successful, NULL otherwise
    ...
    //We won't update the glocal variables throughout the file, so send the message abotu the status
...
/*
    app.css get handler requested when accessing the web page
    req = HTTP request for which the uri needs to be handled
    return = ESP_OK
*/
...
static esp_err_t http_server_get_ap_ssid_json_handler(httpd_req_t *req)
{
    ...
    wifi_config_t *wifi_config = wifi_app_get_wifi_config();
    esp_wifi_get_config(ESP_IF_WIFI_AP, wifi_config); //get wifi info
    char *ssid = (char *)wifi_config->ap.ssid; //extract ssid from wifi info

    sprintf(ssidJSON, "{\"ssid\":\"%s\"}", ssid); //sprintf to buffer
...
/*
    Sets up default httpd server config
    return = http server instance handle if successful, otherwise NULL
*/
static httpd_handle_t http_server_configure(void)
{
    ...
    //Adjust the default priority to 1 less than wifi app task
    ...
    if(httpd_start(&http_server_handle, &config) == ESP_OK)
    {
        ...
        //register favicon.icon handler
        httpd_uri_t favicon_icon = {
            .uri = "/favicon.ico",
            .method = HTTP_GET,
            .handler = http_server_favicon_icon_handler,
            .user_ctx = NULL
        };
        httpd_register_uri_handler(http_server_handle, &favicon_icon);
