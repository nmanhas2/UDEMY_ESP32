...
///	Purpose: WiFi configuration and startup with FreeRTOS tasks
...
//Queue handle used to manipulate main queue of events
...
/*
    ...
    event_id = id of the event to reigster the handler for
...
/*
    Configures the WiFi AP settings and assigns the static IP to the SoftAP
*/
static void wifi_app_soft_ap_config(void)
{
    ...
    //clear out info first
    memset(&ap_ip_info, 0x00, sizeof(ap_ip_info)); //memset copies the character c to the first n characters of the string pointed to

    esp_netif_dhcps_stop(esp_netif_ap); //call this first
    inet_pton(AF_INET, WIFI_AP_IP, &ap_ip_info.ip); //assigns AP's static IP, GW, and netmask
    inet_pton(AF_INET, WIFI_AP_GATEWAY, &ap_ip_info.gw); 
    inet_pton(AF_INET, WIFI_AP_NETMASK, &ap_ip_info.netmask);
    ESP_ERROR_CHECK(esp_netif_set_ip_info(esp_netif_ap, &ap_ip_info)); //statically configs network interace
    ESP_ERROR_CHECK(esp_netif_dhcps_start(esp_netif_ap)); //starts the AP GHCP server for connecting stations (ex: mobile device)
    
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_APSTA)); //setting the mode as AP/ Station mode
    ESP_ERROR_CHECK(esp_wifi_set_config(ESP_IF_WIFI_AP, &ap_config)); //set out configuration
        ...
        if(xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
        {
            switch(msg.msgID)
            {
                ...
                case WIFI_APP_MSG_CONNECTING_FROM_HTTP_SERVER:
                    ...
                    //Let the HTTP server know abotu the connection attempt
                ...
                case WIFI_APP_MSG_STA_CONNECTED_GOT_IP:
                    ...
                    if(eventBits & WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT) // Save STA creds only if connecting from the http server (not loading from NVS)
                    {
                        xEventGroupClearBits(wifi_app_event_group, WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT); //clear bits in case we ewant to disconnect, reconnect, then start again
                    }   
                    else
                    {
...
void wifi_app_start(void)
{
    ...
    //event bits are used to indicate if an event has occurred or not. Sometimes referrerd to as event flags
    //An event group is a set of event bits, individual bits within an event grou are referenced by a bit number
